...
def get_data_from_file(file_path='/Users/rickroma/Desktop/BIA660D/Assignment_01/assignment_01.data'):
...
def select_trip(place):
    for trip in trips:
        if trip.name == name:
            return trip

def add_trip(type, name=None):
    trip = None

    if name:
        trip = select_trip(name)

    if trip is None:
        trip = Trip(place, date)
        trips.append(trip)
...
def process_relation_triplet(triplet):
    ...
    # Process (Person, going, flying, traveling, place)
    if root.lemma_ == 'go':
        if triplet.subject in [e.text for e in doc.ents if e.label_ == 'PERSON'] and triplet.object in [e.text for e in doc.ents if e.label_ == 'OBJECT']:
            s = add_person(triplet.subject)
...
def main():
    ...
    if q_trip.subject.lower() == 'who' and q_trip.object == 'Japan':
        answer = '{} is flying to {}.'

        for person in persons:
            trip = get_persons_trip(person.name)
            if trip and trip.type == 'Japan':
